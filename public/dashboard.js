...
function addCash() {
  ...
  if (amount !== null) {
    ...
    if (!isNaN(num) && num > 0) {
      let currentNetWorth = parseFloat(localStorage.getItem('netWorth')) || 0;
      let updatedNetWorth = currentNetWorth + num;

      localStorage.setItem('netWorth', updatedNetWorth.toString());
      document.getElementById('netValue').innerText = updatedNetWorth.toFixed(2);

      // Show popup
      showPopup(`₹${num} added successfully! New Balance: ₹${updatedNetWorth.toFixed(2)}`);
    } else {
      showPopup("❌ Invalid amount entered.");
    }
  }
}

function showPopup(message) {
  const popup = document.getElementById("cashPopup");
